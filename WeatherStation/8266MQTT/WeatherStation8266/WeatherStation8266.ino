...
String MQTT_CLIENT_ID="weatherStation1";
String MQTT_CLIENT_PWD="station123456";

unsigned long timer = 0;

char* ssid = "pankey_asus";
char* password = "1234554321";
...
void checkWifi() {
  WiFiClient client;
  if (!client.connect("192.168.50.109", 19093)) {
      digitalWrite(12, LOW);
...
void loop() {
    ...
    unsigned long minus = timer - currentTime;
    unsigned long another = currentTime - timer;
    Serial.println(timer);
    Serial.println(currentTime);
    Serial.println(minus);
    if ((timer > currentTime) && (minus > 10000)) {
      Serial.println("timer > currentTime reset");
      timer = 0;
    }
    if (another >= 3000) {
      Serial.println("3s already start publish data");
      // checkWifi();
      if (currentTime >= 86400000) { // restart everyday
        ESP.restart();
      }
      timer = currentTime;

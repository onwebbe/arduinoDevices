...
#define DEVICE_NAME "ESP8266Client-521b"
#define HEADBEAT_CHANNEL "homedevice/switch/ESP8266Client-521b/state"
#define SEND_CHANNEL "homedevice/switch/ESP8266Client-521b/set"
#define RECEIVE_CHANNEL "homedevice/switch/ESP8266Client-521b/set"
#define DISCOVER_CHANNEL "homedevice/switch/ESP8266Client-521b/config"
...
String CLIENT_ID = "ESP8266Client-521b";
...
WiFiManager wm;
OneButton button(RESET_NETWORK_BUTTON_PIN, true);

int currentStatus = 1; // swith off for initialize
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if ((char)payload[0] == '1' && (char)payload[0] == '0') {
    ...
    if (currentStatus == 1) {
      Serial.println("Switch from OFF to ON");
      digitalWrite(LIGHT_PIN, HIGH);  // Turn the LED off by making the voltage HIGH
      digitalWrite(BUILTIN_LED, LOW);
      currentStatus = 0;
    } else {
      Serial.println("Switch from ON to OFF");
      digitalWrite(LIGHT_PIN, LOW);  // Turn the LED off by making the voltage HIGH
      digitalWrite(BUILTIN_LED, HIGH);
      currentStatus = 1;
    }
  }
...
String getPublishContent() {
  ...
  String otherPart = "\",\"pl_on\": \"0\",\"pl_off\": \"1\"}";
  String pubContent = namePart  + DEVICE_NAME + uniqueIdPart + DEVICE_NAME + cmd_TPart + RECEIVE_CHANNEL + stat_TPart + HEADBEAT_CHANNEL + otherPart;
  return pubContent;
}
...
//  Serial.println();
  ...
  client->publish(HEADBEAT_CHANNEL, "online");
  Serial.print("Publish heart beat: ");
  Serial.print(HEADBEAT_CHANNEL);
  Serial.print(":");
  Serial.print("online");
  Serial.println();
...
void setup() {
  ...
  button.attachClick(resetNetworkButtonClicked);
  setup_wifi();

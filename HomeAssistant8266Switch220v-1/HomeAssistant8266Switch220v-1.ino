...
#define DEVICE_NAME "DingDeng1"
#define CLIENT_ID "Smart_Switch_1"
#define CLIENT_PASSWORD "Smart_Switch_123456"


#define SWITCH_PIN 16
#define TEMPERATURE_PIN A0

#define NETWORK_RESET_PIN 12

unsigned int Rs = 250000;//R1取值为150k
double Vcc = 3.0;
...
HAMqttSwitch switch1(client, SWITCH_PIN, "light", DEVICE_NAME, "homedevice/log", true);
HAMqttSensor temperatureSensor(client, "light_temperature", DEVICE_NAME, "homedevice/log");
HAMqttSensorConfig* temperatureConfig;

OneButton networkResetButton(NETWORK_RESET_PIN, true);

ICACHE_RAM_ATTR void networkResetInterrupt() {
...
void setup() {
  ...
  // when switch on, default set the switch on

  
  attachInterrupt(digitalPinToInterrupt(NETWORK_RESET_PIN), networkResetInterrupt, CHANGE);
  networkResetButton.attachClick(resetNetworkButtonClicked);
  ...
  temperatureSensor.setPublishGap(10000);
  temperatureConfig = new HAMqttSensorConfig("℃", "temperature");
  temperatureSensor.addSensorConfig(temperatureConfig);
...
void reconnectMqtt() {
  ...
  while(!client->connected()) {
    ...
    String mqttServerIP = wm.getClientIP();
    String mqttServerPort = wm.getClientPort();
    Serial.print("reconnecting MQTT with: ");
  ...
  temperatureSensor.doSubscribe();
  temperatureSensor.publishDiscover();
  temperatureSensor.publishData();
}
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  temperatureSensor.callback(topicString, payloadString);
}



int checkNetworkTimeGap = 1000;
int checkNetworkTime = millis();
int setTemperatureTimeGap = 1000;
int setTemperatureTime = millis();
void loop() {
  if (millis() - checkNetworkTime > checkNetworkTimeGap) {
    if (!client->connected()) {
      Serial.println("loop-noconnect-Set pin to 0 ");
      switch1.beforeReconnect();
      temperatureSensor.beforeReconnect();
      // wm.reconnect();
      reconnectMqtt();
      switch1.afterRconnected();
      temperatureSensor.afterRconnected();
    }
    checkNetworkTime = millis();
  }

  if (millis() - setTemperatureTime > setTemperatureTimeGap) {
    int tempPin = analogRead(TEMPERATURE_PIN);
    temperatureConfig->setData(String(getTemperature(tempPin)));
    setTemperatureTime = millis();
  }
